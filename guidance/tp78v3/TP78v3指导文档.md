...
| 2026/6/28 | 适配固件版本 V3.3.3 并更新指导文档格式 |
...
**Capslock 设计规则**：层 1 的 Capslock 位置在层 2 必须也是 Capslock 按键，这个按键会在键盘上电自动修改布局。
...
- **TO(n) 切层**：按下后切到第 n 层
- **MO(n) 切层**：按下后切到第 n 层，抬起回到原来的层，但不具有 Capslock 的按键抬起错层判定逻辑。**【当前功能调试中，不可使用】**
...
TP78 支持 modtrack VIA 网页改键。

1. 打开 modtrack VIA 改键网址：<https://via.modtrack.top/>
2. 点击 **Authorize device**（若已连接上直接跳过此步）
3. 参考旧教程的第 3、4 点
...
**Reference 功能**：介绍每个项目的特性和文档参考

![Reference](images/docx_img/img_rId17.png)

**Magnet 功能**：磁轴设定功能，如果键盘为磁轴键盘则该项可以调节磁轴相关的配置

![Magnet](images/docx_img/img_rId18.png)

**GenFirmware 功能**：根据 layout 配置自动改配列生成新固件功能

![GenFirmware](images/docx_img/img_rId19.png)

### 适用 TP78 的特殊按键（后续可能更新）

![特殊按键](images/docx_img/img_rId20.png)
...
| Mouse Right | 鼠标右键 |
| Mouse Middle | 鼠标中键 |
| TP Map Scroll | 按下后小红点垂直移动切换成滚轮上下移动 |
| Fn | 功能键 |

### 旧教程（仅供参考 - 图例以 v2 为标准）

1. Layout 布局获取方法：下载 **TP78 Integrated Tools** 后自动同步最新布局文件到当前目录
2. 打开 VIA 改键网址：<https://usevia.app/>

![旧教程1](images/docx_img/img_rId21.png)

3. 第一次使用需要点击刷子 (Design) 工具，之后点击 **Load** 导入 TP78 的键盘布局文件 (`TP78v3_layout.json`)。注意：如果找不到刷子图标，在设置中打开 **"Show Design tab"** 选项。

![旧教程2](images/docx_img/img_rId22.png)
![旧教程3](images/docx_img/img_rId23.png)

4. 回到第一张图（若已经导入过布局文件可以跳过第 2 步）并点击 **"Authorize device"**，此时已经进入改键页面。

![旧教程4](images/docx_img/img_rId24.png)

5. 点击任意按键后选择下方的 BASIC 按键直接修改生效并保存。例如在 layer2 层 Z 位置增加按键 A：

![旧教程5](images/docx_img/img_rId25.png)
...
![宏设置](images/docx_img/img_rId26.png)

点击左下方第二个图标可以录入宏按键，TP78 支持单个宏按键实现 6 个按键同时按下的组合。V2 不支持 delay 模式，V3 支持 delay 模式。

![宏录入](images/docx_img/img_rId27.png)

### 键盘配置修改

点击左下方第四个图标可以修改 TP78 配置。

![配置修改](images/docx_img/img_rId28.png)
